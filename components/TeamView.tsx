...
const TeamView: React.FC<TeamViewProps> = ({ onBack }) => {
  const teamMembers = [
    { 
      ...
      bio: 'Visionary behind the Core Devs infrastructure. Expert in scalable backend systems and community growth.'
    },
    { 
      name: 'Shadow.js', 
      role: 'Lead Developer', 
      ...
      bio: 'Node.js wizard specializing in high-performance Discord bots and real-time data streaming.'
    },
    { 
      name: 'Nexus.ui', 
      role: 'Design Lead', 
      discord: '@nexus.design', 
      avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=400&h=400&fit=crop',
      bio: 'Crafting premium digital experiences. Responsible for the unique aesthetic of the Core Devs ecosystem.'
    }
  ];
    ...
    <div className="min-h-screen bg-black text-white animate-in fade-in duration-500 pb-32">
      <div className="max-w-7xl mx-auto px-6 pt-32">
        ...
        <div className="reveal active reveal-down text-center mb-24">
          ...
          <p className="text-indigo-400 font-black uppercase tracking-[0.4em] text-[10px]">The Architects behind the Core Devs Hub</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {teamMembers.map((member, idx) => (
            <div key={idx} className={`reveal active reveal-scale stagger-${idx + 1} glass rounded-[3rem] p-10 text-center group hover:border-indigo-500/30 transition-all duration-500`}>
              <div className="relative mb-8 inline-block">
                <img 
                  src={member.avatar} 
                  className="w-44 h-44 rounded-[2.5rem] mx-auto object-cover ring-4 ring-white/5 group-hover:ring-indigo-500/20 transition-all grayscale group-hover:grayscale-0 shadow-2xl" 
                  alt={member.name} 
                />
